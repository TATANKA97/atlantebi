-- mark_sensitive_semantic_columns
-- Existing semantic drafts must not expose credential-shaped metadata as queryable fields.
with normalized_columns as (
  select
    id,
    lower(
      regexp_replace(
        regexp_replace(physical_name, '([a-z0-9])([A-Z])', '\1 \2', 'g'),
        '[^a-z0-9]+',
        ' ',
        'g'
      )
